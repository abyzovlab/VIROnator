...
rule generate_align_job_file:
    ...
    run:
        ...
        try:
            ...
            if bucket:
                subprocess.run(f"gsutil -q cp scripts/* gs://{bucket}/scripts/ 2>/dev/null || true", shell=True)
                subprocess.run(f"gsutil -q cp config/db_metadata/*.* gs://{bucket}/db_metadata/ 2>/dev/null || true", shell=True)
        ...
        formatted_content = (
            ...
            .replace("{plasmid_contigs_path}", os.path.join(config["ref_dir"], config.get("plasmid_contigs_file", "SnapGene_modified.renamed.contigs.txt")))
...
def get_master_report_path(config):
    ...
    return os.path.join(config["output_dir"], stats_dir_name, report_name)
...
rule generate_flag_difference_crams:
    ...
    full flags vs. noflags comparison mini-reports, and automated IGV PNG snapshots for non-zero difference pairs.
    """
    input:
